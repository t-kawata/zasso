...
1. **質問は必ず、AIが検討した理由・背景・トレードオフ分析を含めた提案として提示し、かつユーザーがYes/NoまたはABC選択肢で回答できる形式にすること。**

   - 良い例（背景説明 → 改行リスト選択肢 → おすすめ＋理由）:
     ```
     認証方式について。JWTを提案します。
     理由: 既存システムとの整合性が高く、ライブラリのエコシステムが
     充実しており、ステートレスで水平スケールに適するためです。
     一方、セッション方式はサーバー管理が容易ですがスケール時に追加設計が必要です。
     
       A) JWT
       B) セッション方式
       C) 両方併用
     
     私は A (JWT) をおすすめします。理由はエコシステムの充実度と
     拡張性が他の方式を大きく上回るためです。いかがでしょうか？
     ```
   - 悪い例（理由がない、選択肢が改行されていない）: 「JWTにしますか？ A) Yes  B) No」
