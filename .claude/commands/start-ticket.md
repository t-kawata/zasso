...
**Phase 2 — Resolve `[::AMBIGUOUS::]` markers (loop until clean):**

Some files may contain syntax patterns the script cannot parse. In that case the script inserts a `[::AMBIGUOUS::]` marker as a fallback. The AI must resolve every such marker before proceeding.
...
If the `--check-ambiguous` command exits with code 1 (unresolved markers found):

1. Read each reported file
2. Identify the first meaningful definition (function, class, struct, variable, or whatever the file's dominant construct is)
3. Insert the annotation comment in the standard format at that location
4. Delete the `[::AMBIGUOUS::]` marker line
5. Re-run `--check-ambiguous` and repeat until exit code 0

The AI must keep looping — resolve → check → resolve → check — until `--check-ambiguous` confirms zero markers. Proceeding to Step 6 with unresolved `[::AMBIGUOUS::]` markers is a defect.
