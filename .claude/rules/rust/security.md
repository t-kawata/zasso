...
## zasso Chain of Trust Security Model

zasso はオーナー事務局を起点とする多層署名構造（マトリョーシカ構造）で動作する：
...
- 同一マシン上で同時に起動できる zasso インスタンスは 1 つのみ
- ロックファイル (`~/.zasso/*.lock`) により物理的な排他制御を行う
...
zasso の JWT 認証を必要とする全 REST API エンドポイントは、原則として `JwtRole::USR` のみアクセスを許可する。BD/APX/VDR ロールは管理機能に限定し、将来的に全廃予定。
