...
# settings.ts 自動生成 — run / build の前に必ず実行される
write-settings:
	rm -f fe/src/configs/settings.ts
	printf '// Auto-generated by Makefile — do not edit\n\nexport const EDITION_SLUG = "%s";\nexport const OS_TYPE = "%s";\n' "$(EDITION)" "$(OS_TYPE)" > fe/src/configs/settings.ts
...
commit:
	...
	@# バージョン情報を src-tauri/Cargo.toml から読み取り、パッチバージョンをインクリメントする。
	@# 同時に tauri.conf.json と fe/package.json のバージョンも同期する。
	@OLD_VERSION=$$(grep '^version' src-tauri/Cargo.toml | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+'); \
	...
	$(SED_I) 's/^version = ".*"/version = "'$$NEW_VERSION'"/' src-tauri/Cargo.toml; \
	$(SED_I) 's/"version": ".*"/"version": "'$$NEW_VERSION'"/' src-tauri/tauri.conf.json; \
	$(SED_I) 's/"version": ".*"/"version": "'$$NEW_VERSION'"/' fe/package.json; \
	git add .; \
	if [ -n "$$PUSH_MSG" ]; then \
