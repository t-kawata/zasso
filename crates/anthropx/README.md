...
- Rust 2024 edition（Rust 1.85 以上）
- Cargo（Rust に同梱）
...
| テスト種別 | 配置場所 | テスト数 | 内容 |
|-----------|---------|:--------:|------|
| Unit tests（単体テスト） | 各モジュール内 `#[cfg(test)] mod tests` | 202 | 設定（config）、ルーティング（routing）、スケジューラ（scheduler）、並行性制限（limiter）、プロトコル変換（translate）、メトリクス（metrics）の各関数を個別に検証 |
| Integration tests（統合テスト） | `tests/mock_server.rs` | 21 | `axum_test` を用いた mock upstream サーバーに対する結合テスト。全 4 エンドポイントの動作と受け入れ基準 10 項目をカバー |
...
| Doc tests（ドキュメントテスト） | 各モジュールの doc comments | 3 | `cargo test` で自動実行されるコードサンプルの検証 |

### 11.3 統合テストの受け入れ基準（Acceptance Criteria）

`tests/mock_server.rs` は 21 のテストで構成され、RFC §12 で定義された全受け入れ基準をカバーします:
