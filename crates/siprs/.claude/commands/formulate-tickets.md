...
このステップは Step 7（チケット統合チェック）を通過した後の候補に対して、依存グラフに基づいてフェーズを設計する。Step 7 で統合しきれなかった単位は個別チケットとして扱う。
...
各チケットは **I/O 境界の単位で区切る**。ただし、その前に Step 7（チケット統合チェック）で過剰分解された候補を統合する。判断フロー:

   Step 4 で列挙された候補（発散）
     ↓
   Step 6 で発散（Expand）
     ↓
   Step 7 で「同じI/O境界なら統合」（収束）
     ↓
...
Step 5〜Step 8 で設計したフェーズを add-phase.js で追加する。ID は 0 から自動採番。
