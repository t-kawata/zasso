...
| **A4 section 1 index** | section 1 carries no reverse index, or the seed's heading count disagrees with the section list of the format it declares | the index sits inside section 1 and the count matches that format's section list |
...
**Causes of a reverse FAIL, by gate**: a directory present with no package (A1, "extra"), a declared package with no directory (A1, "missing"), a write outside the allow-list or a renamed top-level entry (A2), an excerpt the packet owed and did not carry (A3), a missing reverse index or a heading count that disagrees with the seed's own declared format (A4), a broken Allocation Index bijection (A5).

**The heading contract is not relaxed, and it is no longer the only one.** `seed-parse.mjs` still enforces an exact match, but against the section list of the format the seed declares: `SEED_FORMATS` holds one row — the current format, built from `SEED_REQUIRED_SECTIONS` — and a seed that declares nothing, or declares a version the table does not hold, is parsed against the format this run reads. A pattern-2 or pattern-3 subject may carry a seed written by an older conver, so that gap is **reported, not refused**: the run continues and the reconciliation is published under `SEED-COMPATIBILITY.md` beside this report. What the finding never becomes is an acceptance — a fifteenth heading or a transposed title fails exactly as it did before, through the same check. The reverse index still extends section 1 in place rather than becoming section 15, and the forward rotation cannot observe any of this.
