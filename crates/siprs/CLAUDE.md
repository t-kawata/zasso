...
siprs is the voice communication foundation within the zasso ecosystem. It wraps PJSUA (PJSIP) FFI bindings in a safe Rust layer and provides an asynchronous event-driven SIP client. Operating independently of existing OS telephony stacks, it integrates with zasso's identity management and P2P network to deliver fully decentralized voice communication.
...
| SIP Library | PJSUA (PJSIP v2.x) via FFI |
| Async Runtime | Tokio |
| Serialization | serde / serde_json |
| Error Handling | thiserror |
| Testing | rustdoc tests + integration tests |

## Language Protocol

| Context | Language | Reason |
|---------|----------|--------|
| Chat, proposals, explanations | **Japanese** | 100% understanding and approval guarantee quality. Japanese is mandatory only when addressing the user directly. |
