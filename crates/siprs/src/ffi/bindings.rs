...
// Allow non-CamelCase names that match the PJSIP C API naming convention.
...
/// PJSUA success return code (maps to `PJ_SUCCESS` = 0).
...
// ---------------------------------------------------------------------------
// Stub FFI functions (compile-time only — no link symbol needed yet)
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn pj_str_t_null_creates_zero_length() {
        ...
        assert!(s.ptr.is_null(), "null pj_str_t must have null ptr");
    }
