...
impl OpenAIRecognizer {
    ...
    pub fn init_audio(&mut self) -> Result<()> {
        ...
        let streamer = PseudoAsrStreamer::new(oa_backend, tx_streamer, streamer_config, None)?;
...
/// LLM の応答から <result> タグで囲まれた内容を抽出する。
///
/// mycute の extract_result 互換。プロンプトで <result> タグでの出力を要求しているため、
/// 応答からタグを除去して純粋な補正テキストのみを返す。
/// タグが見つからない場合は None（呼び出し元で元のテキストを使用）。
fn strip_result_tags(text: &str) -> Option<String> {
    // <result> タグを探す
    let open_tag = "<result>";
    let close_tag = "</result>";
    ...
    if let Some(start) = text.find(open_tag) {
        ...
        if let Some(end) = text[content_start..].find(close_tag) {
            ...
            if !extracted.is_empty() {
                // 入れ子のタグ（<text> 等）があれば除去する
                // mycute の extract_result 互換: 最初の < から次の > までを削除
                let mut cleaned = String::with_capacity(extracted.len());
...
/// LLM でテキストを補正する
    ///
    /// システムプロンプトは mycute (SYSTEM_PROMPT_JA / SYSTEM_PROMPT_EN) と同一。
    /// ロケールに応じてプロンプトと言語を切り替える。
    fn call_post_correct(&self, text: &str) -> Result<String> {
        let locale = *self.language.lock();

        let (system_prompt, user_prompt) = match locale {
            LocaleCode::En => (
                "You are a high-precision English text editor that corrects text generated by automatic speech recognition (ASR), which may not be entirely accurate.\n\
                ...
                10. Remove information noise and ASR errors: if a word or sentence is clearly contextually impossible or irrelevant to the flow of the utterance (e.g., a sudden emotional statement in a factual date/time monologue), treat it as an ASR error and remove or correct it.\n\
                11. Only remove content when you are highly confident it is an ASR error and the original meaning remains intact. If uncertain, keep the original text as-is.",
                format!("Please correct the following text:\n<text>\n{}\n</text>", text),
            ),
            LocaleCode::Ja => (
                "あなたは自動音声認識（ASR）によってテキスト化された、必ずしも正確ではない日本語のテキストを補正する高精度なエディターです。\n\
                次のルールに従って補正してください。\n\n\
                1. 入力内容の意味や流れを変えない\n\
                2. 句読点の過不足を修正\n\
                3. 誤字脱字を修正\n\
                ...
                10. 漢数字は漢数字でなければならない場合を除いて算用数字に直さなければならない。特に日付や時間など。\n\
                11. 情報ノイズと誤認識を除去する：文脈的に明らかに不可能または不自然な単語・文（例：日付と時刻だけを述べている発話に突然現れた感情表現など）は、誤認識と判断して削除または修正する。\n\
                12. 削除や修正を行う場合は、元の発話の内容や流れが変わらない場合に限る。確信が持てない場合は削除せず、元のテキストを維持する。",
        ...
        // mycute 互換: <result> タグで囲まれている場合は内容のみを抽出
        // 前後の空白は補正テキストとして意味を持たないため trim する
        Ok(Self::strip_result_tags(&raw).unwrap_or(raw).trim().to_string())
    }
}
