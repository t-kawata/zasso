...
usage() {
  cat <<'USAGE'
Usage:
  zed-lmpx-launch.sh [-- <zed args...>]

Examples:
  ./zed-lmpx-launch.sh -- ~/src/my-project

What it does:
  - sets Claude Code / Claude Agent environment variables to use
    DeepSeek's Anthropic-compatible endpoint (via lmpx)
  - launches Zed from the same environment

Notes:
  - lmpx のエンドポイントは認証トークンを検証しないため、
...
export ANTHROPIC_MODEL="deepseek-anthropic/deepseek-v4-flash"
export ANTHROPIC_DEFAULT_OPUS_MODEL="deepseek-anthropic/deepseek-v4-pro"
export ANTHROPIC_DEFAULT_SONNET_MODEL="deepseek-anthropic/deepseek-v4-flash"
export ANTHROPIC_DEFAULT_HAIKU_MODEL="ternary-bonsai-27b"
export CLAUDE_CODE_SUBAGENT_MODEL="deepseek-anthropic/deepseek-v4-flash"
