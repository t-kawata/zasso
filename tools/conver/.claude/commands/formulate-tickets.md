...
```bash
echo "=== グラフ構造サマリー ==="
node .claude/scripts/rfc-graph/load-rfc-graph.js "$DOC_PATH" || echo "(グラフ構造サマリーなし)"
echo "========================"
```
...
### Step 4: CLAUDE.md の生成 — 設計全体マップの作成
...
### Step 6: チケット統合チェック — I/O 境界による候補の束ね直し

Step 4 で5層モデルに分類した全要素に対して、以下の質問で統合すべき候補を洗い出す：
...
このステップは Step 5（チケット統合チェック）を通過した後の候補に対して、依存グラフに基づいてフェーズを設計する。Step 5 で統合しきれなかった単位は個別チケットとして扱う。
...
各チケットは **I/O 境界の単位で区切る**。ただし、その前に Step 5（チケット統合チェック）で過剰分解された候補を統合する。判断フロー:

   Step 4 で列挙された候補（発散）
     ↓
   Step 5 で「同じI/O境界なら統合」（収束）
     ↓
...
Step 5-6 で設計したフェーズを add-phase.js で追加する。ID は 0 から自動採番。
