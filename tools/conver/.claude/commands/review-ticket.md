...
| `resolve-ambiguous-markers.js` | `--mode=list-definitions --file=<path> --ticket-key=<key>` | **Executed in Step 5 (AMBIGUOUS resolution)**. Read-only: lists all definitions + AMBIGUOUS line as JSON for AI review. |
| `resolve-ambiguous-markers.js` | `--mode=inject-at --file=<path> --ticket-key=<key> --definition-line=<N>` | **Executed in Step 5 (AMBIGUOUS resolution)**. Inserts `[::TICKET::]` annotation before the specified definition line using `buildAnnotation()`, removes all `[::AMBIGUOUS::]` markers. |
...
**Phase 2 — Script injects `[::TICKET::]` annotation (format guaranteed by `buildAnnotation()`):**

```bash
node .claude/scripts/tickets/resolve-ambiguous-markers.js \
  --mode=inject-at --file="<file-path>" --ticket-key="$ARGUMENTS" \
  --definition-line=<AI-chosen-line-number>
```

This inserts a `[::TICKET::]` annotation before the specified definition and removes all `[::AMBIGUOUS::]` markers. The annotation format is always generated by `buildAnnotation()` — **never hand-typed by AI**.
