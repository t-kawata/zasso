...
| `resolve-ambiguous-markers.js` | `--mode=list-definitions --file=<path> --ticket-key=<key>` | **Executed in Step 5a Phase 2a**. Read-only: lists all definitions + AMBIGUOUS line as JSON for AI review. |
| `resolve-ambiguous-markers.js` | `--mode=inject-at --file=<path> --ticket-key=<key> --definition-line=<N>` | **Executed in Step 5a Phase 2b**. Inserts `[::TICKET::]` annotation before the specified definition line using `buildAnnotation()`, removes all `[::AMBIGUOUS::]` markers. |
...
**Phase 2b — Script injects `[::TICKET::]` annotation (format guaranteed by `buildAnnotation()`):**
