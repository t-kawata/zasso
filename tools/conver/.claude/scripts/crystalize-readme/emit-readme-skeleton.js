 ...
 *
 * FRESH-MODE ONLY: this script emits a fresh skeleton. Refine mode preserves the
 * existing README.md and refines it in place — so an existing README.md is never
 * overwritten (the script exits 1 instead).
 */
...
/**
 * Write the skeleton to the internally-derived README path.
 *
 * FRESH-MODE ONLY: an existing README.md means a previous run's artifact must be
 * refined in place, not reset to a skeleton — so this refuses to overwrite it.
 *
 * @param {Object} status — CRYSTALIZE-Status.json content
 * @returns {string} The skeleton text written
 * @throws {Error} If README.md already exists (refine mode preserves it)
 */
// [::TICKET::] PX-156 changes. Details: `node .claude/scripts/tickets/show-ticket-context.js --ticket-key=PX-156 --for-spec --no-implementation-order`.
function emitSkeletonToFile(status) {
  const readmePath = deriveReadmePath(status);
  if (fs.existsSync(readmePath)) {
    throw new Error(`Refusing to overwrite ${readmePath}: README.md already exists (refine mode preserves the existing README; skeleton emission is fresh-mode only).`);
  }
