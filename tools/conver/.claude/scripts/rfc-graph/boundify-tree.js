...
function buildDirectoryTree(graph, lang, helpers) {
  const titleToFileName = helpers.titleToFileName;
  ...
  function buildTreeFromRoot(root) {
    ...
    if (BACKBONE_KINDS.has(kind)) {
      const dirName = titleToFileName(node.title || 'unnamed', lang);
      const dirNode = {
        name: dirName.replace(/\.(rs|go|ts)$/, ''),
        type: 'directory',
        kind,
      ...
      for (const inlineChild of inlineChildren) {
        const fileName = titleToFileName(inlineChild.title || 'unnamed', lang);
  ...
  const ruleDrivenNodes = findRuleDrivenNodes(graph, hierarchy, lang, titleToFileName, deduplicateFileNames);
 ...
 */
function findRuleDrivenNodes(graph, hierarchy, lang, titleToFileName, deduplicateFileNames) {
  const nodes = graph.nodes || [];
  const result = [];
  ...
  for (const node of nodes) {
    ...
    if (dirName && !BACKBONE_KINDS.has(node.kind || '')) {
      const fileName = titleToFileName(node.title || 'unnamed', lang);
      result.push({
        name: dirName,
        type: 'directory',
        kind: node.kind || '',
