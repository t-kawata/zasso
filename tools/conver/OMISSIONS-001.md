...
> 生成元: `/Users/kawata/shyme/zasso/tools/conver/OMISSIONS-001.json`
...
### O-001 ! [実装漏れ] §§7 (エントリポイント)

RFC §7 では起動時に model, ticketsPath, maxCount, resolveEvery, pushEnabled, timeoutMs の6項目すべてを console.log で表示すると規定されているが、src/conver.ts の main() では model と ticketsPath の2項目のみ表示している。残り4項目の表示が欠落している。


**該当ファイル**:
- `src/conver.ts`

**解決方法**: RFC §7 のコード例に従い、maxCount, resolveEvery, pushEnabled, timeoutMs のログ出力を main() に追加する
---

### O-002 ! [実装漏れ] §§ ファイルパス要件

RFC § ファイルパス要件では「ACP プロトコルの仕様により、すべてのパスは絶対パスで渡す必要がある。Tickets.json のパスは入力を path.resolve() で絶対パスに変換してから渡す」と規定されているが、cli.ts でも runner.ts でも resolve() の呼び出しが実装されていない。相対パスが渡された場合、ACP セッション内でのパス解決が不安定になる可能性がある。
...
**解決方法**: cli.ts の parseCliOptions() で ticketsPath を resolve() で絶対パスに変換してから CliOptions に格納する。また runner.ts の runLoop() 冒頭で cwd も絶対パスに変換する
---

### O-003  [設計不一致] §§4.1

RFC §4.1 のコード例では loadPendingTickets() が flatMap((p) => p.tickets.map((t) => ({ ...t, phaseId: p.id }))) と phaseId を各チケットに付与する実装になっているが、tickets.ts の公開関数 loadPendingTickets() では phaseId を付与せず、phaseId 情報を失っている。runner.ts の非公開版のみが phaseId を付与しているため、tickets.ts の利用者は phaseId を取得できない。


**該当ファイル**:
- `src/tickets.ts`

**解決方法**: tickets.ts の loadPendingTickets() でも phaseId をチケットに付与する。併せて戻り値の型を拡張し、runner.ts と tickets.ts の重複を排除する
---

### O-004  [不整合] §§2.1

RFC §2.1 では AcpSession.stream の型が acp.NdJsonStream、buildClientApp() の戻り値が acp.MonadClient と定義されているが、実装ではそれぞれ acp.Stream、acp.ClientApp となっている。ACP SDK のバージョン更新に伴う型名の変更と思われるが、RFC と実装の乖離が生じており、保守者が混乱する可能性がある。


**該当ファイル**:
- `src/session.ts`

**解決方法**: RFC の型定義を実装に合わせて更新する。または session.ts にコメントでバージョン差異を明記する
---

### O-005  [不整合] §§7 (Makefile エントリ)

RFC §7 では build-conver と run-conver の2つの Makefile エントリのみ定義されている。しかし実際の Makefile には test-conver（node --test で全テスト実行）と list-tickets（チケット一覧表示）のエントリが追加実装されている。RFC と実装の乖離により、新しい開発者が Makefile の全エントリを把握できない。


**該当ファイル**:
- `Makefile`

**解決方法**: RFC §7 に test-conver と list-tickets のエントリを追記し、Makefile の全エントリを RFC に反映する
---
