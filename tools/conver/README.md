...
6. 一つ一つのチケットに対して、/make-ticket -> /plan-ticket -> /start-ticket -> /review-ticket（実装パイプライン）を手動実行
7. 目で追いきれなくても諦めず、AIが仕事する様子を全て読み続けよ（頭で理解しようとするな。カラダで感じよ。）
8. 3チケットくらいの単位で /resolve-ticket を手動実行
9. 目で追いきれなくても諦めず、AIが仕事する様子を全て読み続けよ（頭で理解しようとするな。カラダで感じよ。）
10. 全てのチケットの実装パイプラインが完了したら、/consolidate-stubs -> /find-omissions を連続手動実行
...
※ 次の周のラウンドが必要と判断したら、上記 5 以降を繰り返す。
