...
- The file is **not** in `COMMAND_FILE_NAMES` (`command-file-digest.mjs:50`), which freezes
  **fifteen** command files. `command.test.mjs:145` asserts this explicitly:
...
- But eight structural assertions **do** bind it. They are defined in
  `helpers/command-file.mjs:270` (`assertCommandFileStructure`) and applied to this file at
  `command.test.mjs:153`. A rewrite must keep
  all of them:
...
rewrote the file into a nine-step procedure. **The file is 343 lines now** — P26-2 rewrote it from a description into a procedure, giving every
Step a purpose, a command, a gate, the advice and return Step on failure, and a record — and the absence section
...
## Step 6: author the design semantics     ← P26-5: the readings an author writes, each an inference citing the measured claims it rests on
## Step 7: inspect, repair and reinforce the semantics   ← P26-5: the locator prints where to look; the judgement stays the author's
...
Assertion 7 (`/Step \d/`) is satisfied by `## Step 0` … `## Step 10`.
...
gate reports that a Step of the procedure did not produce what it must, and §1.2 says nothing about
it: that is a fact about this run, not a verdict on the project. The first version of the command
file defined "gate" as the refusal kind, and so forbade both; its `## Statuses and gates` section
stated the absence of gates, every one of its eight uses of the word was a negation, and the reader
was left with no checkpoint anywhere in Steps 0–8. P26-2 gives every Step a progress gate whose
failure names what is wrong, what to do, and the Step to return to.
...
| `run.mjs regression check` as a precondition | It takes **no root** (`.claude/scripts/workspacify-reverse/run.mjs:383-387` — the branch fixes `action: second, root: process.cwd()` and states that requiring a root would make the command unrunnable by the automated sessions that run it before every later ticket's step). It measures the conver repository, and its fixtures are not installed into a user's project |
...
| Whether the four patterns genuinely converge to an identical terminal structure | **Measured once, and not proved.** The chain was driven over all four pattern representatives and the resulting trees were measured against §2.3's inventory, compared dimension by dimension, and recorded in `tests/workspacify-reverse/analysis/TERMINAL-STATE.json` — which names each run's decisions input and its digest. Every representative stopped short, each at a named stage (`T4`, `G2`, `G3`, `G2`), so no terminal state was produced and the comparison has nothing to compare. The claim in §2 therefore remains a design claim, but it is now a claim with a measurement standing beside it rather than a claim nobody had ever tested. What remains unmeasured is whether a convergence holds once a representative *does* reach the terminal state — this observation records that question rather than settling it, and repeating it after the refusing stages are addressed is what several loop rounds means |
