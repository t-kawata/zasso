...
**役割**: RFCを正典とし、RFCの設計内容とルートディレクトリ（RFCファイルがあるディレクトリ）配下の実装コードを比較しながら、開発を収束させるための漏れ・矛盾・不足を発見して `OMISSIONS-XXX.json` に出力する。
...
| スクリプト | 引数 | 説明 |
|---|---|---|
| `create-omissions.js` | `<RFC_FILE_PATH>` | スケルトン生成 + 番号採番 + 検証一体化 |
...
```bash
RESULT=$(node .claude/scripts/tickets/create-omissions.js "$RFC_PATH")
echo "$RESULT"
# {"success":true, "path":"...", "omissionsFilePath":".../OMISSIONS-001.json", "nextNumber":1}
```

出力の `omissionsFilePath` を `OMISSIONS_PATH` として保持する。この時点で `OMISSIONS-XXX.json` には全 `rfcUnderstanding` フィールド（空文字）と全 `steps` が初期値として書き込まれている。
...
全比較ステップ完了後、発見・記録された全 omission に対して機械的フィルタリングを実行する。3つの決定論的スクリプトが直列パイプラインとして動作し、重複排除・Goal阻害度評価・発散傾向判定を自動処理する。機械的フィルタリングは、機械的に点検できる部分のみを点検し有益な情報を提供するだけであり何らかの判断や答えを提供するものではない。機械的フィルタリングによって得られた情報は、RFCの目的・ゴール・成功条件達成に対し、開発全体を収束させていくために、発見・記録された omission をどのように扱うべきかを判断するための情報提供の一つとして扱う。
