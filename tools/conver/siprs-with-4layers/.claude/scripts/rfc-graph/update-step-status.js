...
function executeEndStep(status, n) {
  ...
  if (n >= MAX_STEP) {
    ...
    console.log(`Step ${n}  completed. Status: ${STATUS_DONE}. Next: Step ${n + 1} .`);
  }
}
...
function executeFailStep(status, n) {
  ...
  console.log(`Step ${n}  terminated abnormally. Status: ${STATUS_ERROR}. currentStep is ${status.currentStep}. Check the error, fix the issue, then re-run with reset-to-step ${n}.`);
}
...
function printUsage() {
  console.log(`
update-step-status.js — GRAPHIFY-Status.json / BOUNDIFY-Status.json management
