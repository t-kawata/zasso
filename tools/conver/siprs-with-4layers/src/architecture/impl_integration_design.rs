...
pub fn known_graph_node_ids() -> &'static [&'static str] {
    &[
        "N0008", "N0011", "N0013", "N0015", "N0016", "N0017", "N0018", "N0020", "N0023", "N0025",
        "N0027", "N0028", "N0031", "N0033", "N0034", "N0043", "N0044", "N0067",
    ]
}
...
mod tests {
    ...
    fn residue_root_causes_are_defined() {
        assert_eq!(
            RESIDUE_ROOT_CAUSES.len(),
            3,
            "must cover exactly the 3 RFC §62.0 root causes"
        );
    ...
    fn root_cause_data_is_part_of_module() {
        ...
        assert!(
            r1.evidence.contains("src/config.rs"),
            "R1 evidence must point at the legacy config"
        );
        assert!(
            r1.title.contains("ClientConfig"),
            "R1 title must name ClientConfig"
        );
    }
    ...
    fn io_boundary_table_covers_all_sections() {
        assert_eq!(
            ...
            8,
            "§62.9 table has exactly 8 design decisions"
        );
        let decisions: Vec<DesignDecisionId> =
            IO_BOUNDARY_TABLE.iter().map(|row| row.decision).collect();
        for step in breaking_change_order() {
            assert!(
                decisions.contains(step),
                "section {} must be present in IO_BOUNDARY_TABLE",
                step.section()
            );
        }
    }
    ...
    fn r1_mentions_config_divergence() {
        let r1 = &RESIDUE_ROOT_CAUSES[0];
        assert!(
            r1.evidence.contains("config.rs"),
            "R1 evidence must mention the legacy config file"
        );
        assert!(
            r1.evidence.contains("turn_server"),
            "R1 evidence must mention the turn_server type bug"
        );
    }
    ...
    fn r1_references_n0013_divergence() {
        let r1 = &RESIDUE_ROOT_CAUSES[0];
        assert!(
            r1.rfc_ref.contains("N0013") || r1.rfc_ref.contains("§10"),
            "R1 rfc_ref must reference the ClientConfig spec node"
        );
    }
    ...
    fn r1_describes_turn_server_type_bug() {
        ...
        assert!(
            r1.evidence
                .contains("turn_server: Option<StunServerConfig>"),
            "R1 must record the TURN-as-STUN type bug"
        );
        assert!(
            r1.title.contains("ClientConfig"),
            "R1 title must name the public ClientConfig"
        );
    }
    ...
    fn known_nodes_include_section61_and_clientconfig() {
        let known = known_graph_node_ids();
        assert!(
            known.contains(&"N0067"),
            "§61 I/O boundary reference node N0067 must be known"
        );
        assert!(
            known.contains(&"N0013"),
            "ClientConfig spec node N0013 must be known"
        );
    }
    ...
    fn delete_targets_reference_real_files() {
        assert_eq!(
            DELETE_TARGETS.len(),
            3,
            "§62.9 lists exactly 3 delete targets"
        );
        for target in &DELETE_TARGETS {
            assert!(
                std::path::Path::new(target.path).exists(),
                "delete target {} must exist on disk",
                target.path
            );
        }
    }
    ...
    fn breaking_change_order_is_complete_and_unique() {
        ...
        assert_eq!(
            order.len(),
            8,
            "breaking change order covers sections 62.1..62.8"
        );
